...
@app.route("/api/get_orgs", methods=["POST", "OPTIONS"])
def get_orgs():
    ...
    def generate():
        ...
        payload = {"page": 1, "per_page": 10, "q_organization_name": org_name}
        headers = {
                ...
                result.append(
                    {
                        ...
                        "id": account.get(
                            "organization_id"
                        ),  # Use organization_id for accounts
        ...
        try:
            while partial_results_only:
                ...
                if people:
                    ...
                    page += 1
                    partial_results_only = response_data.get(
...
def get_linkedin_descriptions():
    ...
    def generate():
        ...
        for url in linkedin_urls:
            ...
            if profile:
                descriptions = get_company_descriptions(
...
def get_company_descriptions(profile, company_name):
    ...
    logging.info(
        f"Extracted descriptions for {company_name} from profile: {descriptions}"
    )
    return descriptions
...
def summarize_profiles():
    ...
    if not descriptions_with_citations:
        return jsonify(
            {
                "error": "No relevant descriptions found for the given company and profiles."
            }
        )
...
if __name__ == "__main__":
    ...
    try:
        supabase_response = (
            supabase.table("linkedin_profiles").select("count", count="exact").execute()
        )
        logging.info(
            f"Supabase connection successful. Row count: {supabase_response.count}"
        )
    except Exception as e:
